...
class AdaINStyleTransfer(nn.Module):
    ...
    def forward_with_face_aware_adain(self, content, style, face_masks, 
                                      face_preservation_alpha=0.3, alpha=1.0):
        """
        Perform style transfer with face-aware AdaIN (Regional Adaptive Normalization).
        
        🎨 KEY INNOVATION: Different stylization strengths for face vs. background!
        
        MOTIVATION (from Ulyanov et al.):
        Standard AdaIN applies uniform stylization everywhere, which can distort faces.
        Face-aware AdaIN applies:
        - LIGHTER stylization to face regions (preserve identity)
        - FULL stylization to background (maximum artistic effect)
        - SMOOTH transition at boundaries (no hard edges)
        
        HOW IT WORKS:
        1. Apply standard AdaIN to get fully stylized features
        2. Blend with original content features based on face masks:
           - Face regions: More content (face_preservation_alpha controls blend)
           - Background: Full stylization
           - Smooth gradient at edges (from mask blur)
        ...
        Args:
            ...
            style: (B, 3, H, W) style images in [0, 1] range
                  Example: (4, 3, 512, 512) - 4 art images
            
            face_masks: (B, 1, H, W) binary masks in [0, 1] range
                       1.0 = face region (lighter stylization)
                       0.0 = background (full stylization)
                       Smooth gradient at boundaries
                       Generated by FaceDetector.generate_face_masks()
            
            face_preservation_alpha: Float in [0, 1], stylization strength in face regions
                                    0.0 = Full stylization everywhere (same as baseline)
                                    0.3 = 30% stylization, 70% content in faces (RECOMMENDED)
                                    0.5 = Half stylization in faces (moderate preservation)
                                    1.0 = No stylization in faces (pure content preservation)
                                    
                                    Recommended: 0.3 for best balance
                                    - Preserves identity (face structure, features)
                                    - Still applies artistic style (colors, textures)
                                    - Natural-looking (not copy-pasted face)
            
            alpha: Global style strength (0-1, default 1.0)
                  Applied uniformly everywhere, affects both face and background
                  Usually keep at 1.0 when using face-aware AdaIN
        
        Returns:
            stylized: (B, 3, H, W) stylized images in [0, 1] range
                     Face regions: Lighter stylization (identity preserved)
        ...
        Example:
            ...
            face_masks = detector.generate_face_masks(content_images, margin_factor=1.3)
            
            # Apply face-aware style transfer
            result = model.forward_with_face_aware_adain(
                content=content_images,
                style=style_images,
                face_masks=face_masks,
                face_preservation_alpha=0.3  # 30% stylization in faces
            )
            
            # Result:
            # - Faces: Recognizable, with subtle artistic touches
            # - Background: Fully stylized (Peter Rabbit watercolor effect)
            # - Transition: Smooth gradient (hair, ears gradually stylized)
        
        Technical Details:
            The blending happens at the FEATURE level (not pixel level):
            
            1. Standard AdaIN features: stylized_feat = AdaIN(content_feat, style_feat)
            2. Resize masks to feature resolution (e.g., 256x256 → 32x32)
            3. Blend: output_feat = masks * preserved_feat + (1 - masks) * stylized_feat
               where preserved_feat = (1 - α) * content_feat + α * stylized_feat
            
            This is more effective than pixel-level blending because:
            - Features capture semantic content (what's in the image)
            - Pixels are low-level (just colors)
            - Blending at feature level preserves facial structure better
        """
        # ========================================
        # Step 1: Encode both images to feature space
        # ========================================
        content_feat = self.encode(content)   # (B, 512, H/8, W/8)
        style_feat = self.encode(style)       # (B, 512, H'/8, W'/8)
        
        # ========================================
        # Step 2: Apply standard AdaIN (full stylization)
        # ========================================
        stylized_feat = self.adain(content_feat, style_feat)
        ...
        # Resize face masks to match feature map resolution
        # Example: (B, 1, 256, 256) → (B, 1, 32, 32)
        _, _, feat_h, feat_w = content_feat.shape
        face_masks_resized = F.interpolate(
            face_masks, 
            size=(feat_h, feat_w), 
            mode='bilinear', 
            align_corners=False
        ...
        # Blend based on masks:
        # - Face regions (mask=1): Use preserved_feat (lighter stylization)
        # - Background (mask=0): Use stylized_feat (full stylization)
        ...
        # ========================================
        stylized = self.decode(output_feat)
        
        return stylized
    
    def forward_with_features_face_aware(self, content, style, face_masks,
                                         face_preservation_alpha=0.3, alpha=1.0):
        """
        Face-aware style transfer with intermediate features (for training).
        
        Combines face-aware AdaIN with feature extraction for loss computation.
        This is the training version of forward_with_face_aware_adain().
        
        Args:
            content: (B, 3, H, W) content images
            style: (B, 3, H, W) style images
            face_masks: (B, 1, H, W) binary face masks
            face_preservation_alpha: Stylization strength in face regions (0-1)
            alpha: Global style strength (0-1)
        
        Returns:
            stylized: (B, 3, H, W) stylized image
            stylized_features: dict of features from stylized image
            content_features: dict of features from content image
            style_features: dict of features from style image
        
        Example during training:
            # Generate face masks
            face_masks = face_detector.generate_face_masks(content_imgs)
            
            # Forward pass with face-aware AdaIN
            stylized, stylized_feats, content_feats, style_feats = \
                model.forward_with_features_face_aware(
                    content_imgs, style_imgs, face_masks,
                    face_preservation_alpha=0.3
                )
            
            # Compute losses (same as standard training)
            content_loss = calc_content_loss(stylized_feats, content_feats)
            style_loss = calc_style_loss(stylized_feats, style_feats)
            total_loss = content_loss + 10.0 * style_loss
        """
        # Perform face-aware style transfer
        stylized = self.forward_with_face_aware_adain(
            content, style, face_masks, face_preservation_alpha, alpha
        )
